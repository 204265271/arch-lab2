...
bool 
Lab2PerceptronBP::prediction(int yield){
  return (yield >= 0);
}
...
Lab2PerceptronBP::lookup(ThreadID tid, Addr branch_addr, void *&bp_history) 
{
  ...
  unsigned long long tempGHR = ghr[tid];
...
Lab2PerceptronBP::update(ThreadID tid, Addr branch_addr, bool taken,
  ...
  unsigned pInd = getPerceptronIndex(tid, history->branch_addr);
  unsigned long long rectGHR = history->tempGHR << 1 | taken;
  if (((history->yield >= 0) != taken || abs(history->yield) <= threshold) && history->branch_addr)
    for (int i = 0; i < weightNum; i++){
      bool check = (rectGHR & 1) == taken;
      rectGHR >>= 1;
